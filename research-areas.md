---
layout: page
title: "Research Areas"
permalink: /research-areas/
---

<style>
.research-section {
  max-width: 860px;
  margin: 0 auto;
  padding: 20px 6px 72px;
  color: #111827;
}

.pipeline {
  margin-bottom: 54px;
}

.pipeline h2 {
  position: relative;
  display: inline-block;
  font-size: clamp(1.42rem, 2.2vw, 1.78rem);
  line-height: 1.22;
  margin: 0 0 24px;
  font-weight: 800;
  letter-spacing: -0.035em;
}

.pipeline h2::after {
  content: "";
  display: block;
  width: 46px;
  height: 3px;
  margin-top: 10px;
  border-radius: 999px;
  background: #2f6f9f;
}

.research-list {
  list-style: none;
  padding: 0;
  margin: 0;
}

.research-list li {
  display: grid;
  grid-template-columns: 48px 1fr;
  gap: 20px;
  align-items: start;
  padding: 14px 0;
  border-bottom: 1px solid #e5e7eb;
}

.research-list li:hover {
  background: #f9fafb;
}

.research-number {
  width: 38px;
  height: 38px;
  border-radius: 50%;
  display: inline-flex;
  align-items: center;
  justify-content: center;
  background: #f0f2f4;
  color: #111827;
  font-size: 0.82rem;
  font-weight: 800;
}

.research-title {
  margin: 0;
  max-width: 780px;
  font-size: 1.01rem;
  line-height: 1.5;
  color: #1f2937;
  letter-spacing: -0.01em;
}

.research-year {
  color: #64748b;
  font-size: 0.95rem;
  white-space: nowrap;
}

.research-status {
  color: #2f6f9f;
  font-size: 0.95rem;
  font-weight: 650;
  white-space: nowrap;
}

@media (max-width: 640px) {
  .research-section {
    padding: 18px 4px 56px;
  }

  .pipeline {
    margin-bottom: 46px;
  }

  .pipeline h2 {
    font-size: 1.34rem;
  }

  .research-list li {
    grid-template-columns: 42px 1fr;
    gap: 14px;
    padding: 13px 0;
  }

  .research-number {
    width: 34px;
    height: 34px;
    font-size: 0.78rem;
  }

  .research-title {
    font-size: 0.97rem;
  }
}
</style>


<section class="research-section">


  <div class="pipeline">

    <h2>Political Behavior and Public Opinion</h2>

    <ul class="research-list">

      <li>
        <span class="research-number">01</span>
        <p class="research-title">
          The More the Worse: An Effect of Childcare on Parents’ Environmental Attitudes
          <span class="research-year">(2025)</span>
        </p>
      </li>

      <li>
        <span class="research-number">02</span>
        <p class="research-title">
          Different Clocks, Different Votes: How Temporal Framing Makes Time Matter for Voting
          <span class="research-status">(under review)</span>
        </p>
      </li>

      <li>
        <span class="research-number">03</span>
        <p class="research-title">
          The Paradox of Successful Responses to COVID-19 in South Korea
          <span class="research-year">(2025, book chapter)</span>
        </p>
      </li>

      <li>
        <span class="research-number">04</span>
        <p class="research-title">
          An Empirical Study on the Gender Conflict of the Young Generation: Focusing on Meritocracy and Luck-Egalitarianism
          <span class="research-year">(2022)</span>
        </p>
      </li>

      <li>
        <span class="research-number">05</span>
        <p class="research-title">
          Analysis of the Effect of Youth Allowance on Trust in Local Government
          <span class="research-year">(2022)</span>
        </p>
      </li>

      <li>
        <span class="research-number">06</span>
        <p class="research-title">
          The Effects of Citizens’ Policy Evaluation on Government Trust: Analyzing the Decline of Government Trust during the Late Moon Jae-in Administration through Panel Data
          <span class="research-year">(2022)</span>
        </p>
      </li>

      <li>
        <span class="research-number">07</span>
        <p class="research-title">
          Universal Basic Income and Issue Ownership in South Korea
          <span class="research-year">(2020)</span>
        </p>
      </li>

    </ul>

  </div>


  <div class="pipeline">

    <h2>Party Politics and Political Participation</h2>

    <ul class="research-list">

      <li>
        <span class="research-number">01</span>
        <p class="research-title">
          Party Membership Motivations and Political Activism in a Polarized New Democracy: Evidence from South Korea
          <span class="research-year">(2026)</span>
        </p>
      </li>

      <li>
        <span class="research-number">02</span>
        <p class="research-title">
          Why Do Party Members Leave? Perceived Party Unresponsiveness and Party-Member Exit
          <span class="research-status">(under review)</span>
        </p>
      </li>

      <li>
        <span class="research-number">03</span>
        <p class="research-title">
          Why Do Party Members Make Political Contributions? Motives across Electoral and General Contexts
          <span class="research-status">(under review)</span>
        </p>
      </li>

      <li>
        <span class="research-number">04</span>
        <p class="research-title">
          Is the Korean Party System Unstable after Democratization? Focusing on the Effective Number of Parties and Electoral Volatility Indexes
          <span class="research-year">(2019)</span>
        </p>
      </li>

      <li>
        <span class="research-number">05</span>
        <p class="research-title">
          Analysis on the Causes of Over-Reporting of Voting and Ways to Curb It: Focusing on the Lying-Suspicious Group
          <span class="research-year">(2018)</span>
        </p>
      </li>

    </ul>

  </div>


  <div class="pipeline">

    <h2>Political Representation and Youth Politics</h2>

    <ul class="research-list">

      <li>
        <span class="research-number">01</span>
        <p class="research-title">
          Translating Youth Participation into Policy: A Case Study of the Seoul Youth Ordinance in South Korea
          <span class="research-year">(2026)</span>
        </p>
      </li>

      <li>
        <span class="research-number">02</span>
        <p class="research-title">
          A Representative Pathway to Democratic Deepening: Dual Accountability Linkage and Minority Inclusion
          <span class="research-status">(working paper)</span>
        </p>
      </li>

      <li>
        <span class="research-number">03</span>
        <p class="research-title">
          Voter Bias and Youth Representation: The Role of Prejudice and Age Discrimination
          <span class="research-status">(working paper)</span>
        </p>
      </li>

      <li>
        <span class="research-number">04</span>
        <p class="research-title">
          Between Tradition and Neoliberalism: Temporal Layers of Youth Right-Wing Movements in South Korea
          <span class="research-status">(working paper; intended as a 2027 book chapter)</span>
        </p>
      </li>

      <li>
        <span class="research-number">05</span>
        <p class="research-title">
          The Rise of the South Korean Alt-Right: The “Silent Counter-Revolution” of Young Men
          <span class="research-year">(2025)</span>
        </p>
      </li>

    </ul>

  </div>


</section>
